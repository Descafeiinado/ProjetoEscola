...
void handle_list_teachers_sorted_by_name(char *message) {
  fflush(stdin);
  clear_screen();
  ...
  if (amount > 0) 
    qsort(teachers, amount, sizeof(Person), compare_persons_by_name);

  fflush(stdin);
  clear_screen();

  printf("Listagem de professores por ordem alfabetica:\n\n");
